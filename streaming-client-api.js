...
// Setup and initiate streaming
  ...
  const sessionResponse = await fetch(`${DID_API.url}/talks/streams`, {
    ...
    body: JSON.stringify({
      source_url: "https://raw.githubusercontent.com/jjmlovesgit/D-id_Streaming_Chatgpt/main/gina.png",
    }),
  });
  ...
  try {
    ...
    const requestBody = {
      script: {
        type: 'text',
        subtitles: 'false',
        provider: { type: 'microsoft', voice_id: 'en-CA-ClaraNeural' },
        ssml: false,
        input: chatResponse  // Send the chatResponse to D-ID en-CA-ClaraNeural
      },
      config: {
        fluent: true,
        pad_audio: .0,
...
//  talkVideo.src = 'gina.mp4';
//  talkVideo.loop = true;
//}

function playIdleVideo() {
  talkVideo.srcObject = undefined;
  talkVideo.src = 'ginam3.mp4';
  talkVideo.loop = true;
  talkVideo.muted = true; // Muting the video to ensure autoplay works across all browsers
  talkVideo.play().catch(e => console.error('Error trying to autoplay video:', e));
}
...
document.addEventListener('DOMContentLoaded', () => {
  ...
  buttons.forEach(button => {
    button.addEventListener('click', () => {
      if (talkVideo.muted) { // Check if the video is currently muted
        talkVideo.muted = false; // Unmute the video
      }
    });
  });
});
